...
source: merged render (reads `nb03_fingerprints`, and loads `nb01_fingerprints` + `nb01_circuits` for the intro panels a, b)

message: A BFT fingerprint is the vector of factor loadings a stimulus evokes. On the legible 8x4 even/odd MLP it is a class-structured code (a) whose columns are the circuits BFT finds (b); on the CIFAR-10 CNN the 156-d fingerprint stays class-structured (c) and is as class-separable as the network's own 256-d penultimate activations (d, e), with a clean block class geometry (f).

caption: \textbf{The BFT fingerprint: what it is, and that it is a class-structured, class-separable code.} \textbf{(a)} A fingerprint on the legible $784\to8\to4\to2$ MLP: mean loading per digit over the 13 tree factors, grouped by circuit (blue even, red odd) and layer --- each digit loads almost only its own circuit. \textbf{(b)} The same thing as a graph --- the two circuits those columns come from (solid excitatory, dashed inhibitory; node area $\propto$ loading). \textbf{(c)} On the CIFAR-10 CNN, mean fingerprint per class over the 156 factors, grouped into the ten output circuits; the strip sums each block and every class puts its largest share on its own circuit (framed; $0.22$ against $0.10$ chance). \textbf{(d,~e)} First two principal components of the fingerprints and of the network's own $256$-d penultimate activations, dots colored by class: at $60\%$ of the dimensions the fingerprint is the more class-separable code (silhouette $0.54$ against $0.44$). \textbf{(f)} Pairwise cosine of the 600 test fingerprints, sorted by class (within-class cosine $0.77$). The 8x4 MLP's own fingerprint analysis is Fig.~C; the CNN's is Fig.~F.
