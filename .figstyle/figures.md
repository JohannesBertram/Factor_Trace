...
message: A stimulus's BFT fingerprint is a compact, class-structured code that is more digit-separable than any raw activation layer of the same network, reports the network's own decision on digits the factorization never saw, and collapses to a single point on far-OOD input.

caption: \textbf{The 13-dimensional BFT fingerprint is a class-structured code that reports what the network does --- including on stimuli the factorization never saw.} \textbf{(a)} Mean fingerprint per digit; columns are the tree's factors grouped by circuit (blue even, red odd) and layer. Each digit loads almost only its own circuit, with a distinct $L_1$ profile. \textbf{(b)} Digit separability against the network's own activations: at 13 dimensions the fingerprint has the highest silhouette ($0.56$ against $0.32$ for the 8-d $L_1$ activations and $0.28$ for all 796 activations together) at a $5$-NN accuracy of $0.98$, within $0.02$ of the 784-d pixel and 796-d activation controls. \textbf{(c)} For the six held-out digits (2,\,5--9) the odd circuit's share of the fingerprint tracks the network's own $P(\text{odd})$, not the true parity ($r=0.999$; open circles are the trained digits). \textbf{(d)} Cosine of each fingerprint to its condition's mean: in-distribution and held-out stimuli stay spread out, far-OOD inputs collapse onto a single fingerprint (exactly $1.00$ for the constant and checkerboard images).

## appendix/figC_mlp_fingerprint_details.pdf  (appendix, full width)

source: `notebooks/01_MLP_8_4_0134.ipynb`, §5b

message: The fingerprint machinery behind Fig. 4 is well conditioned — the NNLS projection recovers the NMF's own fingerprint, every condition's mean fingerprint is readable, and the digit blocks behind the Fig. 4(b) silhouette are visible.

caption: \textbf{Fingerprint details for the $8\times4$ even/odd MLP.} \textbf{(a)} NNLS round-trip: projecting the test set onto the fixed factors recovers the fingerprint the NMF itself produced (mean cosine $0.994$, $98.3\%$ above $0.99$; counts log-scaled). \textbf{(b)} Mean fingerprint of every condition, the full version of Fig.~4(a); all four far-OOD types load the same single odd-circuit factor. \textbf{(c)} First two principal components of the fingerprints. \textbf{(d)} Pairwise cosine similarity, 100 stimuli per digit --- the block structure behind the silhouettes of Fig.~4(b): $0.89$ by class, $0.56$ by digit. \textbf{(e)} Maximum cosine to a trained-digit centroid, per condition.

## fig5_digit_mlp_fingerprints.pdf  (main text, full width)

source: `notebooks/02_MLP_40_20_digits.ipynb`, §5a

message: At ten classes the fingerprint stays class-structured and still reports the network's own decision on Fashion-MNIST, but it is no longer the most separable code — the 20-d penultimate activations are. Here it buys traceability, not separability.

caption: \textbf{At ten classes the fingerprint stays class-structured and still reports the network's own decision, but it is no longer the most separable code.} \textbf{(a)} Mean fingerprint per digit over the 125 factors, grouped by the seven output-layer circuits $f_0$--$f_6$ of Fig.~3. \textbf{(b)} Digit separability against the network's own activations. Unlike Fig.~4(b), the 20-d penultimate activations are the most separable representation here (silhouette $0.82$ against the fingerprint's $0.56$, $5$-NN $0.99$ against $0.98$); on this model the fingerprint buys interpretability and traceability, not separability. \textbf{(c)} On Fashion-MNIST the nearest in-distribution digit fingerprint agrees with the network's predicted digit ($r=0.90$ over the 100 class$\times$digit cells; $70\%$ per sample against $10\%$ chance) --- including the network's own idiosyncrasy of funnelling six of the ten clothing classes onto digit~0 and three onto digit~2. \textbf{(d)} As in Fig.~4(d), far-OOD inputs collapse onto a single fingerprint (mean cosine to the condition mean $\ge0.999$ for constant and checkerboard images, against $0.54$ in distribution).

## appendix/figD_digit_mlp_fingerprint_details.pdf  (appendix, full width)

source: `notebooks/02_MLP_40_20_digits.ipynb`, §5b

message: Details for Fig. 5 — the NNLS round-trip is looser here than in the 8x4 net (125 factors, mean cosine 0.92), every condition's mean fingerprint is readable, and Fashion-MNIST falls inside the digit geometry rather than outside it.

caption: \textbf{Fingerprint details for the $40\times20$ digit MLP.} \textbf{(a)} NNLS round-trip. With 125 factors the projection is a looser fit than in the $8\times4$ net (mean cosine $0.919$, min $0.341$, $61\%$ above $0.95$, against $0.994$ in Fig.~C): the more factors the fingerprint has, the less uniquely the non-negative least squares problem is determined. \textbf{(b)} Mean fingerprint of every condition, the full version of Fig.~5(a); the far-OOD conditions again concentrate on a handful of factors. \textbf{(c)} Block cross-similarity of the ten digits and the ten Fashion-MNIST classes: the clothing blocks sit \emph{inside} the digit geometry, not outside it, which is why the fingerprint still names a digit for them. \textbf{(d)} Pairwise cosine similarity, 40 stimuli per digit --- the block structure behind the silhouette of Fig.~5(b). \textbf{(e)} Maximum cosine to a digit centroid, per condition.
...
message: The fingerprint transfers to the transformer unchanged: a 75-d code whose three blocks are the output circuits, class-structured on MNIST (parity silhouette 0.32, digit 0.11 — weaker than the MLPs, and reported as such), recovered almost exactly by the NNLS projection (r = 0.998–0.999), and showing the same OOD signature as every other model — Fashion-MNIST stays inside the in-distribution geometry while far-OOD stimuli collapse onto a single fingerprint. This is the only ViT fingerprint figure of the paper.

caption: \textbf{BFT fingerprints of the TinyViT.} \textbf{(a)} Mean fingerprint per digit over the 75 factors, grouped into the three output circuits $f_0$--$f_2$ of Fig.~G; the strip on the right sums each block: digits 1,\,7,\,9 load $f_0$, digits 3,\,5 load $f_1$, digits 0,\,4 load $f_2$ --- the circuit structure of the trace is visible in the code. \textbf{(b)} Pairwise cosine similarity, 30 stimuli per digit (silhouette $0.32$ by parity, $0.11$ by digit --- lower than the MLPs of Figs.~4 and~5). \textbf{(c)} Mean fingerprint of every stimulus condition; projecting held-out stimuli onto the fixed factors reproduces the NMF's own loadings ($r=0.998$--$0.999$ over the root factors). \textbf{(d)} Mean fingerprint similarity between conditions: no OOD condition comes close to the odd fingerprint ($0.17$ for Fashion-MNIST, $0.08$--$0.11$ for far-OOD), while the four far-OOD conditions are nearly interchangeable ($0.85$--$0.97$). \textbf{(e)} First two principal components of the fingerprints. \textbf{(f)} Cosine of each fingerprint to its condition's mean: in-distribution stimuli stay spread out (median $0.67$), far-OOD inputs collapse onto one fingerprint (median $1.00$ for uniform and checkerboard). \textbf{(g)} Maximum cosine to a trained-class centroid, per condition.
