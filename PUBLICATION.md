...
- **nb03 (CIFAR) died after §3** in the first pass; the §4–§9 rerun landed 2026-09-09 (`59481a9`) and **closed the gap**: `nb03_fingerprints` (aligned `act`, 143 dims), `nb09_cnn_cifar_validation`, and a fresh `nb14_pruning_cnn_cifar` (10 obs, p=0.002, replacing the restored interim copy) are all in. One wrinkle: nb03's act baseline spans the full 2000-stimulus population while the fingerprint rows are a 1500-stimulus subsample — `paper_figures.fp_vs_act` now gathers the act rows at `fp.id_index` (label agreement verified), so the pairing is exact.
...
- **Fresh headline numbers** (checklist complete, 2026-09-09, native-dim silhouettes): even/odd parity 0.925 / digit-structure **0.383** (as predicted); digit MLP **0.665** (up from 0.554, beats dim-matched act 0.221 by +0.444); **CIFAR 0.665** [0.652, 0.682] — the repair confirmed (was 0.362, now beats native-dim act 0.360 by +0.306 with P(fp>act)=1.000; within/between cosine 0.817/0.223); **ImageNet 0.432** [0.410, 0.467] — the feared drop to ~0.30 did not materialize (old two-tree was 0.476; fp beats matched act 0.206 with P(fp>act)=1.000); ViT 0.208 parity / 0.137 digit (still the feasibility case). Cross-model paired tests: fp vs dim-matched act 3/5 (even/odd and ViT lose — the known caveats, frame per the comparison rules above); weight-term control **5/5** arbor-NMF > activation-NMF (p=0.031). CIFAR purity CI computes with real error bars from `stim_labels` (output-layer 0.507); NMF stability min 0.816 (one layer below the 0.85 gate — same as before, mention in text).
- **Figures: 12/12 regenerated** from the final bundle set. Small code changes in this pass: fig6 takes purity labels from the circuit bundle's own `stim_labels`, `fp_vs_act` handles the act-superset alignment via `fp.id_index`, `render_figures.py` and `compute_checklist_stats.py` skip missing bundles gracefully instead of aborting.
